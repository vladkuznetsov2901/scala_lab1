...
object IntHelper:
  ...
  def descriminant(a: Int, b: Int, c: Int): Unit = {
    val descr = (b ^ 2) - 4 * a * c
    var x1 = 0.0
    var x2 = 0.0
    println("Descr: " + descr)
    if (descr >= 0) {
      ...
      val koren = Math.sqrt(Math.abs(descr))
      if  (koren * koren == Math.abs(descr)) {
        val x = "(" + -b + "+-" + Math.abs(koren) + "i" + ")" + " / " + 2 * a
        println(x)
      } else {
        val x = "(" + -b + "+- √" + Math.abs(descr) + "i" + ")" + " / " + 2 * a
        println(x)
      }
